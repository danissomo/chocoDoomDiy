...
SDL_Renderer* DisplayManager::Init(const std::string& sWindowTitle, int iWindowHeight, int iWindowWidth) {
  ...
  m_pTexture = SDL_CreateTexture(m_pRenderer, SDL_PIXELFORMAT_ARGB8888,
...
void DisplayManager::Render(){
  SDL_SetPaletteColors(m_pScreenBuffer->format->palette, m_ColorPalettes[0].colors, 0, 256);
...
void DisplayManager::AddColorPalette(WADPalette &palette){
    m_ColorPalettes.push_back(palette);
}
